...
#RUN BEST MODEL
mod <- occu(~ Effort
            ~ scale.forest_score_50m_radius+scale.CT_distance_from_plantations+site, 
            data = umf)
...
siteraster <- raster("C:/Users/guest/Dropbox (ScreenForBio)/Projects/moonrat/Prediction_rasters/site_raster_clipped.tif")
#TRIraster <- raster("C:/Users/guest/Dropbox (ScreenForBio)/Projects/moonrat/TRI_not_smoothed_proper_scale_150m_clipped.tif")
...
site.s <- siteraster
site.s <- as.factor(site.s)
#TRI.s <- (TRIraster-7.3678241)/4.8369076

#ef <- stack(fs.s, plant.s, site.s)

ef <- data.frame(coordinates(fs.s),
                 scale.forest_score_50m_radius = values(fs.s),
                 scale.CT_distance_from_plantations = values(plant.s),
                 site= values(site.s))
#names(ef) <- c("scale.forest_score_50m_radius", "scale.CT_distance_from_plantations","site")
ef$site <- as.factor(ef$site)
ef <- ef[which(!is.na(ef$scale.forest_score_50m_radius)),]
...
logit.psi <- beta[1] + beta[2]*fs.s + beta[3]*plant.s + beta[4]*site.s + beta[5]*site.s ####
...
#save(E.psi, file="PredictionResults_COARSE_P(Effort)_fs+disttoplants+site.RData")
